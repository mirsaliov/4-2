 ...
 * Файл ssd1306.c
 * ---------------
 * Это нижний уровень OLED-драйвера SSD1306.
 *
 * Здесь находятся функции, которые реально работают с дисплеем:
 * - отправка команд SSD1306;
 * - инициализация дисплея;
 * - очистка и заполнение видеобуфера;
 * - отправка buffer[] на OLED;
 * - рисование пикселя, линии, прямоугольника и bitmap.
 *
 * Важно понимать:
 * функции рисования НЕ отправляют данные сразу на OLED.
 * Они изменяют только display->buffer[].
 * Чтобы увидеть результат на дисплее, нужно вызвать SSD1306_Update()
 * или добавить команду SSD1306_SetUpdateCommand().
 */
